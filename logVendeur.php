	...
	if(isset($_POST['button1'])) {
		...
		if($db_found){	
        	...
        	if(empty($nom) || empty($mdp) || empty($prenom) || empty($mail) ) {
				header("Location: loginVendeur.php?signup=empty");    
			} elseif(!preg_match("/^[a-zA-Z]*$/", $nom) || !preg_match("/^[a-zA-Z]*$/", $mdp) || !preg_match("/^[a-zA-Z]*$/", $prenom)){
					header("Location: loginVendeur.php?signup=char"); 
					exit();
			
					} elseif(!preg_match("#^[a-z0-9._-]+@edu.ece.fr$#", $mail)){
						header("Location: loginVendeur.php?signup=mail&nom=$nom&prenom=$prenom"); 
						exit();
			
					}elseif(strcmp($mdp,$mdp2)!==0){ 
						header("Location: loginVendeur.php?signup=mdp&nom=$nom&prenom=$prenom"); 
						exit(); 
					}else{      
        					$checkslq = "SELECT `Mail` FROM `vendeur` WHERE `Mail` LIKE '$mail'";
								...
								if(mysqli_num_rows($result) != 0){
									header("Location: loginVendeur.php?signup=mail_used"); 
									exit();	
									...
									header("Location: loginVendeur.php?signup=success"); 
									exit();
								}
							}
